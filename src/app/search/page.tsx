...
export async function generateMetadata(
  ...
  return {
    title: query ? `Search: ${query} | Swaply` : "Search | Swaply",
    description: `Search results for "${query}" on Swaply`,
  };
}
...
async function searchListings(query: string): Promise<Listing[]> {
  ...
  const { data, error } = await supabase
    .from("listings")
    .select("id, title, price, city, description, images, image_urls, created_at")
